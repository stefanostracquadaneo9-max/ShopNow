...
async function initializeStripeCheckout() {
    if (typeof window.isStaticCheckoutMode === "function" && window.isStaticCheckoutMode()) {
    ...
    try {
        ...
        const configResponse = await window.fetchWithTimeout(window.getApiUrl("/config"), {
            headers: window.getApiRequestHeaders(),
        });
        const config = await configResponse.json();
        
        if (!configResponse.ok || !config.stripePublicKey || config.stripePublicKey.includes("placeholder")) {
        ...
        window.showCheckoutMessage("danger", "Impossibile caricare il sistema di pagamento.");
    }
}
...
async function handleCheckoutSubmit(event) {
    ...
    if (!validatePostalCode()) {
        window.showCheckoutMessage("danger", "Il CAP inserito non è valido per il paese selezionato.");
    ...
    try {
        ...
        const intentResponse = await window.fetchWithTimeout(window.getApiUrl("/create-payment-intent"), {
            method: "POST",
            headers: window.getApiRequestHeaders({ "Content-Type": "application/json" }),
            body: JSON.stringify({ amount: total, items, customerName: name, customerEmail: email }),
        });
        const intentData = await intentResponse.json();
        if (!intentResponse.ok) throw new Error(intentData.error || "Errore inizializzazione pagamento.");

        // 2. Conferma pagamento con Stripe
        const paymentResult = await stripeInstance.confirmCardPayment(intentData.clientSecret, {
            payment_method: {
        ...
        const checkoutResponse = await window.fetchWithTimeout(window.getApiUrl("/api/checkout"), {
            method: "POST",
            headers: window.getApiRequestHeaders({
        ...
        if (!checkoutResponse.ok) throw new Error(data.error || "Errore registrazione ordine.");
        ...
        window.showCheckoutMessage("success", `Ordine #${data.order.id} confermato con successo!`);
        if (typeof window.showToast === "function") window.showToast("Pagamento completato!");
...
function configureStaticCheckoutUi() {
    ...
    window.showCheckoutMessage("warning", "Stripe richiede un backend configurato.");
}
