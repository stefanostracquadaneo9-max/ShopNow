...
window.viewUserDetails = async function (userId) {
    ...
    body.innerHTML = '<div class="text-center py-4 text-muted">Caricamento...</div>';
...
window.saveUser = async function () {
    ...
    const password =
        document.getElementById("user-password")?.value.trim() || "";
    const role =
...
window.downloadBackup = async function () {
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(
                data?.error || "Errore durante il download del backup",
            );
        }
                ...
                datasets: [
                    {
                        data: [
                            orders.filter(
                                (order) => String(order.status) === "paid",
                            ).length,
                            orders.filter(
                                (order) => String(order.status) !== "paid",
                            ).length,
